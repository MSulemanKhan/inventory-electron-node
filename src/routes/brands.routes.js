...
router.delete('/delete-all', (req, res) => {
  db.run('DELETE FROM brands', [], function(err) {
    if (err) return res.status(500).send(err);
    res.send({ message: 'All brands deleted', rows: this.changes });
  });
});
...
router.post('/import', upload.single('file'), (req, res) => {
  ...
  try {
    ...
    const tasks = records.map(row => (done) => {
      ...
      db.get('SELECT id FROM brands WHERE name = ?', [name], (e, ex) => {
        if (e) { results.errors++; return done(e); }
        if (ex && ex.id) {
          db.run('UPDATE brands SET description = ? WHERE id = ?', [description, ex.id], function(uerr) {
            if (uerr) { results.errors++; return done(uerr); }
            results.updated++; return done(null);
          });
        } else {
          db.run('INSERT INTO brands(name, description) VALUES (?, ?)', [name, description], function(ierr) {
    ...
    (function runNext(i) {
      if (i >= tasks.length) { try { fs.unlinkSync(filePath); } catch (e) {} return res.send(results); }
      tasks[i]((err) => { if (err) console.error('Import brand row error', err && err.message); runNext(i+1); });
    })(0);
  } catch (e) {
